...
import { CoolifyProvider } from "./providers/coolify-provider";
...
export class SandboxFactory {
	static create(
		...
		switch (selectedProvider.toLowerCase()) {
			...
			case "coolify":
				return new CoolifyProvider(config || {});

			default:
				throw new Error(
					`Unknown sandbox provider: ${selectedProvider}. Supported providers: e2b, vercel, coolify`,
				);
		}
	}

	static getAvailableProviders(): string[] {
		return ["e2b", "vercel", "coolify"];
	}
	...
	static isProviderAvailable(provider: string): boolean {
		switch (provider.toLowerCase()) {
			...
			case "coolify":
				return !!process.env.COOLIFY_API && !!process.env.COOLIFY_ADDRESS;
